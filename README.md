...
本系统在 3.0 版本中引入了**全量统计、精英入库**机制，不再仅仅是搬运 Repo，而是对全球代码演进进行“智力审计”。
...
| 变量名 | 说明 |
| --- | --- |
| `GITHUB_TOKEN` | 必须具备 `contents: write` 权限以存档情报 |
| `REPO_OWNER` | 自动解析，当前为 `wenfp108` |
| `REPO_NAME` | 自动解析，当前为 `git-data` |

---

*Built with ❤️ by **Woon**. Intelligence powered by Unit 4 Sentinel Engine.*
